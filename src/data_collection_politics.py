...
# === CONFIGURATION ===
NUM_POSTS = 100  # 100 posts
NUM_COMMENTS = 1000  # Up to 1000 comments per post
NUM_REPLIES = 10     # Up to 10 replies per comment
...
def scrape_reddit(subreddit_name="politics", sort_by="hot"):
    ...
    posts = subreddit.hot(limit=NUM_POSTS) if sort_by == "hot" else subreddit.top(limit=NUM_POSTS)

    print(f"Scraping {sort_by} posts from r/{subreddit_name}...")
        ...
        try:
            ...
            for comment in top_comments:
                ...
                if len(data) >= BATCH_SIZE:
                    save_batch(data, f"politics_{sort_by}", batch_num)
                    batch_num += 1
                    data = []
